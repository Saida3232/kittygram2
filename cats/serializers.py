...
class CatSerializer(serializers.ModelSerializer):
    ...
    def validate_birth_year(self, value):
        ...
        if not (year - 140 < value <= year):
            raise serializers.ValidationError('Проверьте год рождения котика.он не может быть бессмертным)')
        return value

    def validate(self, data):
        if data['color'] == data['name']:
            raise serializers.ValidationError(
                'Имя котика не может совпадать с цветом'
            )
        return data
